...
const Cart = () => {
  ...
  const totalQty = cartItems.reduce((totalQty, item) => totalQty + item.qty, 0);         /// acc total hota h
  ...
  return (
    <>
      <div
        className={`fixed right-0 top-0 lg:w-[20vw] bg-white h-full p-5 ${
      ...
      <PiShoppingCartFill
        onClick={() => setActiveCart(!activeCart)}
        className={`rounded-full bg-white shadow-md text-5xl p-3 fixed bottom-10 right-10 ${totalQty > 0 && "animate-bounce delay-500 translate-all"}`}
      />
    </>
  );
};
